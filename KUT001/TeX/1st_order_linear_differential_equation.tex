...
Najskôr získame homogénne riešenie. Rovnicu (\ref{1stOderLODE.Equation:1stOrderLinearDifferentialEquation}) zhomogenizujeme tým, že uvažujeme $u(t) = 0$:
\begin{equation}
    \label{1stOderLODE.Equation:1stOrderLinearDifferentialEquationHomogeneous}
    \dot{x}_h(t) = a(t) x_h(t)
\end{equation}

\noindent Zhomogenizovaná rovnica (\ref{1stOderLODE.Equation:1stOrderLinearDifferentialEquationHomogeneous}) je separovateľná:
\begin{equation}
    \label{1stOderLODE.Equation:1stOrderLinearDifferentialEquationHomogeneousSeparated}
    \der{x_h(t)}{t} = a(t) x_h(t) \longrightarrow \frac{dx(t)}{x_h(t)} = a(t)dt
\end{equation}

\noindent Následne integrujeme obidve strany rovnice (\ref{1stOderLODE.Equation:1stOrderLinearDifferentialEquationHomogeneousSeparated}):
\begin{equation}
    \label{1stOrderLODE.Equation:HomogeneousIntegral}
...
Riešenie zhomogenizovanej rovnice (\ref{1stOderLODE.Equation:1stOrderLinearDifferentialEquationHomogeneous}) má teda tvar:
\begin{equation}
    \label{1stOderLODE.Equation:HomogeneousSolution}
    x_h(t_f) = L \exp \left( \int_{t_i}^{t_f} a(t) \, dt \right)
\end{equation}

Teraz prejdeme na vyjadrenie partikulárneho riešenia. Predpokladáme, že bude v rovnakom tvare ako homogénne riešenie (\ref{1stOderLODE.Equation:HomogeneousSolution}):
\begin{equation}
    \label{1stOderLODE.Equation:ParticularSolutionForm}
...
Partikulárne riešenie (\ref{1stOderLODE.Equation:ParticularSolutionForm}) zderivujeme:
\begin{equation}
    \dot{x}_p(t) = 
    \dot{L}(t) \exp \left( \int_{t_i}^{t} a(\tau) \, d\tau \right) + 
    L(t) a(t) \exp \left( \int_{t_i}^{t} a(\tau) \, d\tau \right)
\end{equation}
a dosadíme do (\ref{1stOderLODE.Equation:1stOrderLinearDifferentialEquation}). Po úpravách dostaneme:
\begin{equation}
    \dot{L}(t)  = 
...
a po integrácii:
\begin{equation}
    \label{1stOderLODE.Equation:ConstantVariation}
    L(t_f) + M = \int_{t_i}^{t_f} \exp \left( -\int_{t_i}^{t} a(\tau) \, d\tau \right) b(t) u(t) \, dt
\end{equation}

\noindent Nakoniec dosadením (\ref{1stOderLODE.Equation:ConstantVariation}) do (\ref{1stOderLODE.Equation:ParticularSolutionForm}) ešte vyjadríme partikulárne riešenie $x_p$ v bode $t_f$:
\begin{align}
    \label{1stOderLODE.Equation:ParticularSolution}
    \nonumber
    x_p(t_f) &= 
...
Získané homogénne (\ref{1stOderLODE.Equation:HomogeneousSolution}) a partikulárne (\ref{1stOderLODE.Equation:ParticularSolution}) riešenie teraz sčítame podľa (\ref{1stOderLODE.Equation:SolutionLinearCombination}):
\begin{align}
    \nonumber
    x(t_f) &= 
